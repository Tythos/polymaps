...
define(function(require, exports, module) {
    ...
    (function(po){
        ...
        po.transform = function(a, b, c, d, e, f) {
            var transform = {}, zoomDelta, zoomFraction, k;
            if (!arguments.length) {
                a = 1;
                c = 0;
                e = 0;
            ...
            transform.apply = function(x) {
                ...
                var k1 = Math.pow(2, x.zoom - zoomDelta);
                return {
            ...
            transform.unapply = function(x) {
                var k0 = Math.pow(2, -x.zoom), k1 = Math.pow(2, x.zoom + zoomDelta);
                return {
        ...
        po.cache = function(load, unload) {
            var cache = {}, locks = {}, map = {}, head = null, tail = null, size = 64, n = 0;
            ...
            cache.load = function(c, projection) {
                var key = [c.zoom, c.column, c.row].join("/"), tile = map[key];
                if (tile) {
                    if (tile.prev) {
        ...
        po.url = function(template) {
            var hosts = [], repeat = true;

            function format(c) {
                var max = c.zoom < 0 ? 1 : 1 << c.zoom, column = c.column;
                if (repeat) {
                    column = c.column % max;
                ...
                return template.replace(/{(.)}/g, function(s, v) {
                    switch (v) {
                        ...
                        case "B": {
                            ...
                            var se = po.map.coordinateLocation({row: c.row + 1, column: column + 1, zoom: c.zoom});
                            var pn = Math.ceil(Math.log(c.zoom) / Math.LN2);
        ...
        po.map = function() {
            var map = {}, container, size, sizeActual = zero, sizeRadius = zero; // sizeActual / 2
            var tileSize = {x: 256, y: 256}, center = {lat: 37.76487, lon: -122.41948}, zoom = 12, zoomFraction = 0, zoomFactor = 1; // Math.pow(2, zoomFraction)
            var zoomRange = [1, 18], angle = 0, angleCos = 1; // Math.cos(angle)
            var angleSin = 0; // Math.sin(angle)
            var angleCosi = 1; // Math.cos(-angle)
            var angleSini = 0; // Math.sin(-angle)
            var ymin = -180; // lat2y(centerRange[0].lat)
            var ymax = 180; // lat2y(centerRange[1].lat)
            var centerRange = [
            ...
            map.locationCoordinate = function(l) {
                var c = po.map.locationCoordinate(l), k = Math.pow(2, zoom);
                c.column *= k;
                c.row *= k;
                c.zoom += zoom;
                return c;
            ...
            map.coordinatePoint = function(tileCenter, c) {
                var kc = Math.pow(2, zoom - c.zoom), kt = Math.pow(2, zoom - tileCenter.zoom);
                var dx = (c.column * kc - tileCenter.column * kt) * tileSize.x * zoomFactor;
                var dy = (c.row * kc - tileCenter.row * kt) * tileSize.y * zoomFactor;
                return {
            ...
            map.pointCoordinate = function(tileCenter, p) {
                ...
                var dx = (p.x - sizeRadius.x) / zoomFactor;
                var dy = (p.y - sizeRadius.y) / zoomFactor;
                return {
            ...
            map.locationPoint = function(l) {
                ...
                var dx = (l.lon - center.lon) * k * tileSize.x;
                var dy = (lat2y(center.lat) - lat2y(l.lat)) * k * tileSize.y;
                return {
                    "x": sizeRadius.x + angleCos * dx - angleSin * dy,
                    "y": sizeRadius.y + angleSin * dx + angleCos * dy
                };
            ...
            map.pointLocation = function(p) {
                ...
                var dx = (p.x - sizeRadius.x) * k;
                var dy = (p.y - sizeRadius.y) * k;
                return {
            ...
            function recenter() {
                ...
                var lat0 = y2lat(ymin - y * k / tileSize.y);
                var lat1 = y2lat(ymax + y * k / tileSize.y);
                ...
                var lon0 = centerRange[0].lon - x * k / tileSize.x;
                var lon1 = centerRange[1].lon + x * k / tileSize.x;
            ...
            map.extent = function(x) {
                ...
                var bl = map.locationPoint(x[0]), tr = map.locationPoint(x[1]);
                var k = Math.max((tr.x - bl.x) / sizeActual.x, (bl.y - tr.y) / sizeActual.y);
                var l = map.pointLocation({x: (bl.x + tr.x) / 2, y: (bl.y + tr.y) / 2});
        ...
        po.layer = function(load, unload) {
            var layer = {}, cache = layer.cache = po.cache(load, unload).size(512);
            var tile = true, visible = true, zoom, id, map, container = po.svg("g");
            var transform, levelZoom, levels = {};
            ...
            function move() {
                ...
                var mapZoomFraction = mapZoom - (mapZoom = Math.round(mapZoom));
                var mapSize = map.size(), mapAngle = map.angle();
                var tileSize = map.tileSize(), tileCenter = map.locationCoordinate(map.center());
                ...
                var c1 = map.pointCoordinate(tileCenter, {x: mapSize.x, y: 0});
                var c2 = map.pointCoordinate(tileCenter, mapSize);
                var c3 = map.pointCoordinate(tileCenter, {x: 0, y: mapSize.y});
                ...
                function projection(c) {
                    var zoom = c.zoom, max = zoom < 0 ? 1 : 1 << zoom;
                    var column = c.column % max, row = c.row;
                    if (column < 0) column += max;
                    return {
                ...
                if (visible && tileLevel > -5 && tileLevel < 3) {
                    ...
                    if (tile) {
                        ...
                        var y = Math.max(0, Math.min(ymax - 1, Math.floor((c1.row + c3.row) / 2)));
                        var z = Math.min(4, c0.zoom);
                        x = x >> z << z;
                        y = y >> z << z;
                        scanLine(x, x + 1, y);
                    }
                ...
                function scanLine(x0, x1, y) {
                    var z = c0.zoom, z0 = 2 - tileLevel, z1 = 4 + tileLevel;
                ...
                for (var key in newLocks) {
                    var t = newLocks[key];
                    var k = Math.pow(2, t.level = t.zoom - tileCenter.zoom);
        ...
        function scanSpans(e0, e1, ymin, ymax, scanLine) {
            ...
            // scan lines!
            var m0 = e0.dx / e0.dy, m1 = e1.dx / e1.dy;
        ...
        po.image = function() {
            var image = po.layer(load, unload), url;
        ...
        po.geoJson = function(fetch) {
            var geoJson = po.layer(load, unload), container = geoJson.container(), url;
            var clip = true, clipId = "org.polymaps." + po.id();
            var clipHref = "url(#" + clipId + ")";
            var clipPath = container.insertBefore(po.svg("clipPath"), container.firstChild);
            var clipRect = clipPath.appendChild(po.svg("rect"));
            var scale = "auto", zoom = null, features;
            ...
            var types = {
                "Point": function(o, proj) {
                    var p = proj(o.coordinates), c = po.svg("circle");
                ...
                "MultiPoint": function(o, proj) {
                    var g = po.svg("g"), c = o.coordinates;
                    var p; // proj(c[i])
                    var x; // svg:circle
                    var i = -1, n = c.length;
                    while (++i < n) {
                ...
                "LineString": function(o, proj) {
                    var x = po.svg("path"), d = ["M"], c = o.coordinates;
                    var p; // proj(c[i])
                    var i = -1, n = c.length;
                ...
                "MultiLineString": function(o, proj) {
                    var x = po.svg("path"), d = [], ci = o.coordinates;
                    var cj; // ci[i]
                    var i = -1, j, n = ci.length, m;
                    while (++i < n) {
                        cj = ci[i];
                        j = -1;
                        m = cj.length;
                ...
                "GeometryCollection": function(o, proj) {
                    var g = po.svg("g"), i = -1, c = o.geometries, n = c.length, x;
                    while (++i < n) {
                        x = geometry(c[i], proj);
            ...
            var rescales = {
                ...
                "MultiPoint": function (o, e, k) {
                    var c = o.coordinates, i = -1, n = p.length, x = e.firstChild, p;
                    while (++i < n) {
                        p = c[i];
                    ...
                    switch (data.type) {
                        case "FeatureCollection": {
                            for (var i = 0; i < data.features.length; i++) {
                                ...
                                var element = geometry(feature.geometry, proj);
            ...
            function move() {
                ...
                var tiles = geoJson.cache.locks(); // visible tiles
                var key; // key in locks
                var tile; // locks[key]
                var features; // tile.features
                var i; // current feature index
                var n; // current feature count, features.length
                var feature; // features[i]
                var k; // scale transform
                if (scale == "fixed") {
                    for (key in tiles) {
        ...
        po.dblclick = function() {
            var dblclick = {}, zoom = "mouse", map, container;
        ...
        po.drag = function() {
            var drag = {}, map, container, dragging;
        ...
        po.wheel = function() {
            var wheel = {}, timePrev = 0, last = 0, smooth = true;
            var zoom = "mouse", location, map, container;
            ...
            function mousewheel(e) {
                var delta = e.wheelDelta || -e.detail, point;
        ...
        po.arrow = function() {
            var arrow = {}, key = {left: 0, right: 0, up: 0, down: 0};
            var last = 0, repeatTimer, repeatDelay = 250, repeatInterval = 50;
            var speed = 16, map, parent;
        ...
        po.hash = function() {
                ...
                if (args.length < 3 || args.some(isNaN)) return true; // replace bogus hash
                    ...
                    map.zoomBy(
                        args[0] - map.zoom(),
                        {
                            ...
                            "lat": Math.min(lat, Math.max(-lat, args[1])),
                            "lon": args[2]
                        }
                    );
                }
            ...
            var formatter = function(map) {
                var center = map.center(), zoom = map.zoom();
        ...
        po.touch = function() {
            var touch = {}, map, container, rotate = false, last = 0, zoom;
            var angle, locations = {}; // touch identifier -> location
            window.addEventListener("touchmove", touchmove, false);
            
            function touchstart(e) {
                var i = -1, n = e.touches.length, t = Date.now();
            ...
            function touchmove(e) {
                switch (e.touches.length) {
                    ...
                    case 2: { // double-touch pan + zoom + rotate
                        ...
                        var p0 = map.mouse(t0), p1 = map.mouse(t1);
                        var p2 = { "x": (p0.x + p1.x) / 2, "y": (p0.y + p1.y) / 2}; // center point
                        var c0 = po.map.locationCoordinate(locations[t0.identifier]);
                        var c1 = po.map.locationCoordinate(locations[t1.identifier]);
                        var c2 = {"row": (c0.row + c1.row) / 2, "column": (c0.column + c1.column) / 2, zoom: 0};
                        var l2 = po.map.coordinateLocation(c2); // center location
        ...
        po.interact = function() {
            var interact = {}, drag = po.drag(), wheel = po.wheel();
            var dblclick = po.dblclick(), touch = po.touch(), arrow = po.arrow();
        ...
        po.compass = function() {
            var compass = {}, g = po.svg("g"), ticks = {}, r = 30, speed = 16;
            var last = 0, repeatDelay = 250, repeatInterval = 50;
            var position = "top-left"; // top-left, top-right, bottom-left, bottom-right
            var zoomStyle = "small"; // none, small, big
            var zoomContainer, panStyle = "small"; // none, small
            var panTimer,  panDirection,panContainer, drag, dragRect = po.svg("rect");
            var map, container, window;
            ...
            function pan(by) {
                var x = Math.SQRT1_2 * r, y = r * .7, z = r * .2, g = po.svg("g");
                var dir = g.appendChild(po.svg("path"));
                var chv = g.appendChild(po.svg("path"));
            ...
            function zoom(by) {
                var x = r * .4, y = x / 2, g = po.svg("g");
                var back = g.appendChild(po.svg("path"));
                var dire = g.appendChild(po.svg("path"));
                var chev = g.appendChild(po.svg("path"));
                var fore = g.appendChild(po.svg("path"));
            ...
            function tick(i) {
                var x = r * .2, y = r * .4, g = po.svg("g");
                var back = g.appendChild(po.svg("rect"));
                var chev = g.appendChild(po.svg("path"));
        ...
        po.grid = function() {
            var grid = {}, map, g = po.svg("g");
            g.setAttribute("class", "grid");
            
            function move(e) {
                var p, line = g.firstChild, size = map.size();
                var nw = map.pointLocation(zero);
                var se = map.pointLocation(size);
        ...
        po.stylist = function() {
            var attrs = [], styles = [], title;

            function stylist(e) {
                var ne = e.features.length, na = attrs.length, ns = styles.length;
                var f; // feature
                var d; // data
                var o; // element
                var x; // attr or style or title descriptor
                var v; // attr or style or title value
                var i, j;
                for (i = 0; i < ne; ++i) {
    ...
    return Object.assign(org.polymaps, {
        "__url__": "",
        "__semver__": "2.5.1",
        "__license__": "BSD-3"
    });
